...
public class SpecificationServiceImpl implements SpecificationService {
	...
	public void add(Specification specification) {
		...
		for (TbSpecificationOption specificationOption : specificationOptionList) {
			...
			//本次插入的Id获取到set到规格项中,直接从规格中获取,但是默认插入是不返回id的.需要从mapper中设置
	...
	public Specification findOne(Long id){
		...
		specification.setSpecification(specificationMapper.selectByPrimaryKey(id));//查一

		TbSpecificationOptionExample example = new TbSpecificationOptionExample();
		TbSpecificationOptionExample.Criteria criteria = example.createCriteria();
		criteria.andSpecIdEqualTo(id);
		List<TbSpecificationOption> specificationOptionList = specificationOptionMapper.selectByExample(example);//查多
